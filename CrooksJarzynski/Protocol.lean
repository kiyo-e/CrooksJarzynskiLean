...
/-!
# Finite-state Crooks and Jarzynski theorems

A time step first changes the energy from `E_t` to `E_{t+1}` while the
microstate remains fixed, and then applies a Markov transition at the new
energy.  Local detailed balance is imposed at that post-quench energy.

The main proof first establishes an unnormalized path identity for Boltzmann
weights.  Gibbs normalization is inserted only afterwards.  This separation
keeps the induction transparent and avoids divisions inside the physical local
balance assumption.
-/
...
This is the algebraic core: the initial Boltzmann weight times the forward
transition product equals the final Boltzmann weight times the reverse product
and the exponential work factor.
-/
